...
// 用例 1：网络隔离（NetworkMode: none）。
// 实测修正：network:none 下用域名时 DNS 查询会挂起直到兜底 kill，
...
// 用例 6：死循环 → TLE（宿主兜底计时器，wallMs 略大于 5000）
...
// 用例 7：输出轰炸 → OLE（truncated 标记，采集内存不失控）
...
// 用例 10：残留检查——沙箱容器必须被回收。
// 池化模式下常驻容器（running 的 sleep infinity）是合法存在，
